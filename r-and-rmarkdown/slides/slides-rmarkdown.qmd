...
:::
::: {.column width="50%"}
<center>![](https://hadley.nz/hadley.jpg){width=30%}</center>
<center>图片描述</center>
:::
:::
...
根据上述统计摘要，数据包含 `r knitr::inline_expr("nrow(expr_data)")` 个样本。
````
...
````markdown
本次分析共 `r knitr::inline_expr("nrow(cars)")` 个样本，
平均速度 `r knitr::inline_expr("round(mean(cars$speed), 1)")` mph，
中位刹车距离 `r knitr::inline_expr("median(cars$dist)")` 英尺。
````
...
> Quarto (<https://quarto.org/>)是**下一代开源科学出版系统**，统一支持R、Python、Julia等多语言

::: {.columns}
![](figures/quarto.jpg){width="70%"}
:::
