...
**原理**：Markdown要求列表块前有空行才能被识别为独立块。紧跟在加粗文本后的列表会被视为文本延续而非列表。
...
使用 `<a href="..." download>` HTML标签强制下载而非显示：

````markdown
# ❌ 错误 - 点击后会在浏览器中显示文件内容
[下载数据](data/geneCountMatrix.txt)

# ✅ 正确 - 点击后触发下载
<a href="data/geneCountMatrix.txt" download>下载数据</a>
...
**路径规则**：
- 同级目录：`data/file.txt`
- 子目录到上级：`../data/file.txt`
- 根目录到子目录：`rna-seq-analysis/data/file.txt`
